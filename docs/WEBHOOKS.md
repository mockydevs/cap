...
The `webhook-worker` service polls a transactional outbox (`webhook_outbox`) written by the web app and the media/transcription/AI workers in the same database transaction as the underlying state change, so an event is never emitted for a change that didn't actually commit. It requires `WEBHOOK_SECRETS_KMS_KEY_ARN` — a KMS key dedicated to webhook signing secrets, distinct from the media and AI-credential keys, so no other service can decrypt them.
